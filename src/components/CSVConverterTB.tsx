...
export function CSVConverterTB() {
  ...
  const [tableName, setTableName] = useState(''); // State to hold the table name
  ...
  // Modify the SQL generation to use the user-provided table name
  const generateCreateTableSQL = () => {
    const table = tableName || 'csv_import'; // Default to 'csv_import' if no table name is provided
  ...
  const handleImportData = async () => {
    ...
    try {
      ...
      const insertResponse = await fetch(`${API_BASE_URL}/api/insert-data`, {
        ...
        body: JSON.stringify({
          credentials: mysqlCredentials,
          tableName: tableName, // Use user-provided table name here
          columns: headers,
          data: rows,
        }),
      });
